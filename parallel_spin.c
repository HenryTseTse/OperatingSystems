...
From the results from threads 2 to 8, we can see that mutex is generally quicker. One reason this could be the case is the fact that spinlock is a lock that causes an acquiring thread to wait in a loop while checking for an avaiable lock. Spinlock will have a higher CPU usage while waiting; therefore, it will generally be slower. 

The advantage of using spinlock is the unlock and lock operations, which is relatively cheap without the need for context switching. When using one thread operations run in serial, since there are no lock conflicts this causes spin to have a faster runtime.
